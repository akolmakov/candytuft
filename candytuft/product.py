...
class Store:
	...
	def to_dict(self) -> Dict[str, Any]:
		return {"id": str(self.id), "short_name": self.short_name, "name": self.name, "currency": self.currency, "url": self.url}
...
class Family:
	def __init__(self, id: UUID, foreign_id: str, store_id: UUID, vendor_id: UUID, name: str, url: str, timestamp: Optional[int]):
		self.id = id
	...
	def from_dict(dict: Dict[str, Any]) -> "Family":
		return Family(id=UUID(dict["id"]), foreign_id=dict["foreign_id"], store_id=UUID(dict["store_id"]), vendor_id=UUID(dict["vendor_id"]),
			name=dict["name"], url=dict["url"], timestamp=dict["timestamp"])

	def to_dict(self) -> Dict[str, Any]:
		return {"id": str(self.id), "foreign_id": self.foreign_id, "store_id": str(self.store_id), "vendor_id": str(self.vendor_id), "name": self.name,
...
class Product:
	...
	def get_option(self, name) -> Optional[str]:
